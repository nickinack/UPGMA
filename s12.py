...
def merge_rows(Table , min_1 , min_2,min_dist,headers,branch_lengths):
    ...
    branch_lengths[headers[min_1] + headers[min_2]] = min_dist/2
...
def UPGMA(Table,headers,branch_lengths):
    '''
    UPGMA for x iterations where during iteration x, length of header becomes 1
    '''
    while len(headers)>1 and len(Table)>1:
        min_i , min_j , min_dist = find_min_dist(Table)
        if min_i < min_j:
            merge_rows(Table , min_i , min_j,min_dist,headers,branch_lengths)
        elif min_j < min_i:
            merge_rows(Table , min_j , min_i,min_dist,headers,branch_lengths)
        print(len(Table))
        if len(headers) == 1:
            break
    return headers
...
if __name__ == "__main__":
    Table,headers=create_dist_table('Ndistance.csv')
    branch_lengths = {}
    UPGMA(Table,headers,branch_lengths)
    test_model(headers)
    text_file = open('a12.txt',"w")
